...
from Research_report_spider import Research_report_spider


class YYpingjiSpider(object):
    def __init__(self):
        self.url = 'https://api.ratingdog.cn/v1/autocomplete?'
        self.url2 = 'https://api.ratingdog.cn/v1/search?'  # 用于条件筛选用的url
    ...
    def query_page_select(self):
        ...
        if keyword == '1':
            ...
            return self.YY_level_show()
        elif keyword == '3':
            return self.listed_show()
        elif keyword == '4':
            return self.enterprise_nature_show()
        elif keyword == '5':
            return self.keyword_query_show()

    def YY_level_show(self):
        """
            按照YY等级搜索
        :return:
        """
        select = self.ui.YY_level_page()
        timestamp = str(int(time.time() * 1000))
        filter = None
        if select == ' ':
    ...
    def listed_show(self):
        """
            根据是否已上市来搜索
        :return:
        """
        select = self.ui.listed_page()
        timestamp = str(int(time.time() * 1000))
        filter = None
        if select == ' ':
            self.query_page_select()
    ...
    def enterprise_nature_show(self):
        select = self.ui.enterprise_nature_page()
        timestamp = str(int(time.time() * 1000))
        filter = None
        if select == ' ':
            self.query_page_select()
